...
.header input:invalid {
  ...
  border-style: dashed;
  --tw-border-opacity: 1;
  border-bottom-color: rgba(var(--dyte-colors-danger, 255 45 45) / var(--tw-border-opacity));
}

.header input.editing-enabled {
  --tw-bg-opacity: 1;
...
.header .participant-count dyte-icon {
  margin: var(--dyte-space-0, 0px);
...
.selector-mode:hover .show-on-hover {
  display: flex;
}
...
.message-container p:hover {
  ...
  background-color: rgba(var(--dyte-colors-brand-500, 33 96 253) / 0.25);
}

.message-container .drop-zone-active {
  --tw-border-opacity: 1;
  border-color: rgba(var(--dyte-colors-brand-300, 73 124 253) / var(--tw-border-opacity));
  background-color: rgba(var(--dyte-colors-brand-500, 33 96 253) / 0.25);
}
