...
dyte-message-view::part(message) {
  flex-grow: 1;
}

*[is-continued] dyte-message-view {
...
.pinned .pin-button {
  ...
  top: calc(var(--dyte-space-4, 16px) * -1);
  display: flex;
...
.pinned .pin-button:hover dyte-button {
  --tw-bg-opacity: 1;
...
.show-new-messages {
  pointer-events: auto;
  --tw-translate-y: var(--dyte-space-16, 64px);
