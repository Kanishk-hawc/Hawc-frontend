...
import {
  TextMessageView
} from "./chunk-VVSFZTZB.js";
import {
  Render
} from "./chunk-SKOJFCA3.js";
import {
  hark
} from "./chunk-RBQYBTDM.js";
import {
  SyncWithStore
} from "./chunk-LFTNBGM5.js";
import {
  DyteNotificationsAudio
} from "./chunk-IOVXSAA7.js";
import {
  deepMerge,
  generateConfig,
  provideDyteDesignSystem
} from "./chunk-XOFLUPWT.js";
import "./chunk-HWMB33MY.js";
import "./chunk-BPOQX6WG.js";
...
// node_modules/@dytesdk/ui-kit/dist/esm/dyte-audio-visualizer_64.entry.js
var drawBarsVisualizer = (canvas, volume) => {
  ...
  const sample = [...Array(nSlices)].map((_, i) => {
    let index2 = i;
    if (index2 > halfwaySlice - 1) {
      index2 = nSlices - index2 - 1;
    }
    return Math.round((index2 + 1) / (halfwaySlice + 1) * volume);
  });
...
var ROOM_TITLE_MIN_CHARS = 3;
var DyteBreakoutRoomManager = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
  ...
  connectedCallback() {
    ...
    this.canEditMeetingTitle = this.permissions.canAlterConnectedMeetings && !this.room.isParent && !this.meeting.connectedMeetings.isActive;
...
var DyteBreakoutRoomParticipants = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
  ...
  getParticipants(search) {
    ...
    this.participantsToShow = allParticipants.filter((participant) => {
      var _a;
      return this.participantIds.includes(participantIdentifier(participant)) && ((_a = participant.displayName) !== null && _a !== void 0 ? _a : "").toLowerCase().includes(search.toLowerCase());
    });
  }
  renderPeer(participant) {
    const name = formatName(participant.displayName || "");
...
var MIN_ROOMS = 1;
var DyteBreakoutRoomsManager = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
  ...
  render() {
    ...
    }), h("dyte-button", { kind: "button", variant: "secondary", onClick: this.onAddNewRoom, class: "add-room-jumbo-btn br-secondary-btn" }, h("div", null, h("dyte-icon", { icon: this.iconPack.add }), h("span", null, this.t("breakout_rooms.add_room_brief"))))))), h("div", { class: "participant-config-actions" }, h("div", { class: { "status-bar": true, "ephemeral-status": this.ephemeralStatusText !== "" } }, this.ephemeralStatusText !== "" && h("dyte-icon", { icon: this.iconPack.checkmark }), this.getStatusText()), h("div", { class: "breakout-actions" }, this.roomConfig.mode === "create" && this.permissions.canAlterConnectedMeetings && h("dyte-button", { size: "md", class: "start-breakout-button", onClick: () => this.enableConfirmationModal("start-breakout") }, this.t("breakout_rooms.start_breakout")), this.roomConfig.mode === "edit" && this.stateManager.hasLocalChanges && this.permissions.canAlterConnectedMeetings && h("dyte-button", { size: "md", class: "color-danger", variant: "ghost", onClick: () => this.discardChanges() }, this.t("breakout_rooms.discard_changes")), this.roomConfig.mode === "edit" && this.stateManager.hasLocalChanges && this.permissions.canAlterConnectedMeetings && h("dyte-button", { size: "md", class: "update-breakout-button", onClick: this.applyChanges }, this.t("breakout_rooms.update_breakout")), this.roomConfig.mode === "edit" && !this.stateManager.hasLocalChanges && this.permissions.canAlterConnectedMeetings && h("dyte-button", { size: "md", class: "close-breakout-button", onClick: () => this.enableConfirmationModal("close-breakout") }, this.t("breakout_rooms.close_breakout"))))));
  }
  static get watchers() {
    return {
...
var DyteBroadcastMessageModal = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.stateUpdate = createEvent(this, "dyteStateUpdate", 7);
    this.iconPack = defaultIconPack;
...
var DyteChannelCreator = class {
  constructor(hostRef) {
    ...
    this.step = 1;
    this.loading = false;
    this.selectedMemberIds = /* @__PURE__ */ new Set();
    this.focusedMemberIndex = -1;
    this.showAllMembersList = false;
    this.allMembers = /* @__PURE__ */ new Map();
    ...
    this.renderMemberSelector = () => {
      const filteredMembers = Array.from(this.allMembers.values()).filter((member) => !this.selectedMemberIds.has(member.userId) && member.name.toLowerCase().includes(this.searchQuery.toLowerCase()));
      ...
      }, onKeyDown: (e) => this.keyDownHandler(e, filteredMembers) })), (this.searchQuery.length !== 0 || this.showAllMembersList) && h("ul", { class: "search-results" }, filteredMembers.map((member, index2) => h("li", { class: { member: true, selected: index2 === this.focusedMemberIndex }, onClick: () => this.onMemberAdd(member.userId), ref: ($li) => {
        if (index2 === this.focusedMemberIndex) {
          writeTask(() => {
            if ($li)
...
var DyteChannelSelectorView = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
  ...
  render() {
    ...
    }))), this.viewAs === "dropdown" && h("button", { key: "af78d56510b62beeee9e17162a0fbe5a75b0b1af", class: "dropdown-trigger", onClick: this.toggleDropdown }, h("span", { key: "10c170707e3c084d4a0a1a322128f6e685c075ed" }, this.selectedChannelId && `${this.t("to")} ${this.getChannelById(this.selectedChannelId).name}`, this.getTotalUnreads() > 0 && h("div", { key: "b45263bedbb36f6b25cb595067a2418a110e9e45", class: "unread-count" }, this.getTotalUnreads())), h("dyte-icon", { key: "ca61473ab2f859378fea75468dbf194a137ceba0", icon: this.showDropdown ? this.iconPack.chevron_up : this.iconPack.chevron_down })));
  }
  get $el() {
    return getElement(this);
  }
...
var DyteChat = class {
  constructor(hostRef) {
    ...
    this.onParticipantUpdate = () => {
      this.participants = this.meeting.participants.joined.toArray().filter((p) => this.privatePresetFilter.length === 0 || this.privatePresetFilter.includes(p.presetName));
    ...
    this.onChannelCreateOrUpdate = (channel) => {
      ...
      const channels = allChannels.filter((channel2) => !isDirectMessageChannel(channel2)).sort((a, b) => alphabeticalSorter(a.displayName, b.displayName));
      const membersWithChannel = allChannels.filter(isDirectMessageChannel).map((channel2) => {
        return Object.assign(Object.assign({}, channel2), { displayName: this.getMemberDisplayName(channel2) });
      });
      const membersWithoutChannel = this.meeting.participants.all.toArray().filter((member) => {
        if (member.userId === this.meeting.self.userId)
          return false;
        const matcher = getDMComparator([this.meeting.self.userId, member.userId]);
        return membersWithChannel.every((channel2) => getDMComparator(channel2.memberIds) !== matcher);
      }).map((member) => {
        return {
    ...
    this.getMemberDisplayName = (channel) => {
      ...
      if (channel.memberIds.length === 1) {
        ...
        id = channel.memberIds[0] === this.meeting.self.userId ? channel.memberIds[1] : channel.memberIds[0];
      }
      const member = this.meeting.participants.all.toArray().find((member2) => member2.userId === id);
    ...
    this.getChannelItems = () => {
      return this.channels.map((channel) => {
        ...
        if (channel.latestMessage) {
          result.latestMessage = channel.latestMessage.type === "text" ? stripOutReplyBlock(channel.latestMessage.message) : "";
  ...
  meetingChanged(meeting, oldMeeting) {
    ...
    if (meeting != null) {
      ...
      this.canPinMessages = ((_e = meeting === null || meeting === void 0 ? void 0 : meeting.__internals__) === null || _e === void 0 ? void 0 : _e.features.hasFeature(FlagsmithFeatureFlags.PINNED_MESSAGES)) && meeting.self.permissions.pinParticipant;
  ...
  chatGroupsChanged(chatGroups) {
    ...
    for (const key in chatGroups) {
      ...
      unreadCounts[key] = chatGroups[key].filter((c) => c.type == "chat" && c.message.time > lastReadTimestamp && c.message.userId !== this.meeting.self.userId).length;
      if (key === generateChatGroupKey([this.meeting.self.userId, (_b = this.selectedParticipant) === null || _b === void 0 ? void 0 : _b.userId]) || key === "everyone" && this.selectedParticipant === null) {
        unreadCounts[key] = 0;
  ...
  renderComposerUI() {
    ...
    return h("dyte-chat-composer-view", Object.assign({ message, storageKey: (_a = this.selectedChannelId) !== null && _a !== void 0 ? _a : `draft-${this.selectedChannelId}`, quotedMessage, isEditing: !!this.editingMessage, canSendTextMessage: this.isTextMessagingAllowed(), canSendFiles: this.isFileMessagingAllowed(), disableEmojiPicker: !!((_c = (_b = this.meeting) === null || _b === void 0 ? void 0 : _b.__internals__) === null || _c === void 0 ? void 0 : _c.features.hasFeature(FlagsmithFeatureFlags.DISABLE_EMOJI_PICKER)), maxLength: this.meeting.chat.maxTextLimit, rateLimits: this.meeting.chat.rateLimits, inputTextPlaceholder: this.t("chat.message_placeholder"), onNewMessage: this.onNewMessageHandler, onEditMessage: this.onEditMessageHandler, onEditCancel: this.onEditCancel, onQuotedMessageDismiss: this.onQuotedMessageDismiss }, uiProps), h("slot", { name: "chat-addon", slot: "chat-addon" }));
  }
  renderFullChat() {
...
var DyteChatComposerView = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
...
var dyteChatMessagesUiPaginatedCss = ":host{display:flex;flex-direction:column;--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-background-900, 26 26 26) / var(--tw-bg-opacity));flex:1 0 0px}";
var DyteChatMessagesUiPaginatedStyle0 = dyteChatMessagesUiPaginatedCss;
...
var DyteChatMessagesUiPaginated = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    ...
    this.getChatMessages = async (timestamp, size, reversed) => {
      const { messages } = await this.meeting.chat.getMessages(timestamp, size, reversed, void 0, this.selectedChannelId);
      this.maybeMarkChannelAsRead(messages);
      return messages;
    };
...
var DyteChatSearchResults = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.iconPack = defaultIconPack;
    this.t = useLanguage();
    ...
    this.nodeRenderer = (messages) => {
      return messages.map((message) => h("dyte-chat-message", { key: message.id, message, disableControls: true }));
    };
  }
  render() {
...
var DyteConfirmationModal = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.stateUpdate = createEvent(this, "dyteStateUpdate", 7);
...
var dyteDebuggerScreenshareCss = ":host{line-height:initial;font-family:var(--dyte-font-family, sans-serif);font-feature-settings:normal;font-variation-settings:normal}p{margin:var(--dyte-space-0, 0px);padding:var(--dyte-space-0, 0px)}.scrollbar{scrollbar-width:thin;scrollbar-color:var(--dyte-scrollbar-color, rgb(var(--dyte-colors-background-600, 60 60 60)))\n    var(--dyte-scrollbar-background, transparent)}.scrollbar::-webkit-scrollbar{height:var(--dyte-space-1\\.5, 6px);width:var(--dyte-space-1\\.5, 6px);border-radius:9999px;background-color:var(--dyte-scrollbar-background, transparent)}.scrollbar::-webkit-scrollbar-thumb{border-radius:9999px;background-color:var(--dyte-scrollbar-color, rgb(var(--dyte-colors-background-600, 60 60 60)))}:host{position:relative;display:flex;flex-direction:column;overflow:hidden;height:100%;width:100%;color:rgb(var(--dyte-colors-text-1000, 255 255 255));--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-background-600, 60 60 60) / var(--tw-bg-opacity))}.tab-body::-webkit-scrollbar{width:var(--dyte-space-1\\.5, 6px);border-radius:var(--dyte-border-radius-md, 8px);--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-background-1000, 8 8 8) / var(--tw-bg-opacity))}.tab-body::-webkit-scrollbar-thumb{border-radius:var(--dyte-border-radius-md, 8px);--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-background-700, 44 44 44) / var(--tw-bg-opacity))}h3{margin:var(--dyte-space-0, 0px);padding:var(--dyte-space-0, 0px);font-size:16px;font-weight:600}#header{margin-top:var(--dyte-space-2, 8px);margin-bottom:var(--dyte-space-5, 20px);display:flex;align-items:center;justify-content:space-evenly}.tab-body{margin:var(--dyte-space-4, 16px);display:flex;height:100%;flex-direction:column;justify-content:space-between;overflow-y:auto}.status-container{border:var(--dyte-border-width-sm, 1px) solid rgb(var(--dyte-colors-background-600, 60 60 60));border-radius:8px;padding:16px}.status-section{margin-bottom:12px}.section-header{display:flex;justify-content:space-between;align-items:center;font-weight:bold;padding:8px 0}.section-header.only-child{justify-content:center}.section-body.missing-stats{display:flex;text-align:center;justify-content:center}.section-header .status{color:rgba(var(--dyte-colors-success));font-weight:bold}.section-header .arrow{font-size:14px}.network-table{border:var(--dyte-border-width-sm, 1px) solid rgb(var(--dyte-colors-background-600, 60 60 60));border-radius:8px;margin-top:8px;overflow:hidden}.network-row{display:flex;justify-content:space-between;padding:12px;border-bottom:var(--dyte-border-width-sm, 1px) solid rgb(var(--dyte-colors-background-600, 60 60 60))}.network-row:last-child{border-bottom:none}.network-cell{display:flex;flex-direction:column}.network-cell.label strong{font-size:14px}.network-cell.label .description{font-size:12px;color:rgb(var(--dyte-colors-text-700, 255 255 255 / 0.64));margin-top:4px}.network-cell.value{text-align:right}.status.good{color:rgba(var(--dyte-colors-success))}.status.average{color:rgba(var(--dyte-colors-warning))}.status.poor{color:rgba(var(--dyte-colors-danger))}.value{font-size:14px;color:rgb(var(--dyte-colors-text-700, 255 255 255 / 0.64));margin-top:4px}";
var DyteDebuggerScreenshareStyle0 = dyteDebuggerScreenshareCss;
...
var DyteDebuggerScreenShare = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.iconPack = defaultIconPack;
    this.t = useLanguage();
...
var dyteDebuggerVideoCss = ":host{line-height:initial;font-family:var(--dyte-font-family, sans-serif);font-feature-settings:normal;font-variation-settings:normal}p{margin:var(--dyte-space-0, 0px);padding:var(--dyte-space-0, 0px)}.scrollbar{scrollbar-width:thin;scrollbar-color:var(--dyte-scrollbar-color, rgb(var(--dyte-colors-background-600, 60 60 60)))\n    var(--dyte-scrollbar-background, transparent)}.scrollbar::-webkit-scrollbar{height:var(--dyte-space-1\\.5, 6px);width:var(--dyte-space-1\\.5, 6px);border-radius:9999px;background-color:var(--dyte-scrollbar-background, transparent)}.scrollbar::-webkit-scrollbar-thumb{border-radius:9999px;background-color:var(--dyte-scrollbar-color, rgb(var(--dyte-colors-background-600, 60 60 60)))}:host{position:relative;display:flex;flex-direction:column;overflow:hidden;height:100%;width:100%;color:rgb(var(--dyte-colors-text-1000, 255 255 255));--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-background-600, 60 60 60) / var(--tw-bg-opacity))}.tab-body::-webkit-scrollbar{width:var(--dyte-space-1\\.5, 6px);border-radius:var(--dyte-border-radius-md, 8px);--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-background-1000, 8 8 8) / var(--tw-bg-opacity))}.tab-body::-webkit-scrollbar-thumb{border-radius:var(--dyte-border-radius-md, 8px);--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-background-700, 44 44 44) / var(--tw-bg-opacity))}h3{margin:var(--dyte-space-0, 0px);padding:var(--dyte-space-0, 0px);font-size:16px;font-weight:600}#header{margin-top:var(--dyte-space-2, 8px);margin-bottom:var(--dyte-space-5, 20px);display:flex;align-items:center;justify-content:space-evenly}.tab-body{margin:var(--dyte-space-4, 16px);display:flex;height:100%;flex-direction:column;justify-content:space-between;overflow-y:auto}.status-container{border:var(--dyte-border-width-sm, 1px) solid rgb(var(--dyte-colors-background-600, 60 60 60));border-radius:8px;padding:16px}.status-section{margin-bottom:12px}.section-header{display:flex;justify-content:space-between;align-items:center;font-weight:bold;padding:8px 0}.section-header.only-child{justify-content:center}.section-body.missing-stats{display:flex;text-align:center;justify-content:center}.section-header .status{color:rgba(var(--dyte-colors-success));font-weight:bold}.section-header .arrow{font-size:14px}.network-table{border:var(--dyte-border-width-sm, 1px) solid rgb(var(--dyte-colors-background-600, 60 60 60));border-radius:8px;margin-top:8px;overflow:hidden}.network-row{display:flex;justify-content:space-between;padding:12px;border-bottom:var(--dyte-border-width-sm, 1px) solid rgb(var(--dyte-colors-background-600, 60 60 60))}.network-row:last-child{border-bottom:none}.network-cell{display:flex;flex-direction:column}.network-cell.label strong{font-size:14px}.network-cell.label .description{font-size:12px;color:rgb(var(--dyte-colors-text-700, 255 255 255 / 0.64));margin-top:4px}.network-cell.value{text-align:right}.status.good{color:rgba(var(--dyte-colors-success))}.status.average{color:rgba(var(--dyte-colors-warning))}.status.poor{color:rgba(var(--dyte-colors-danger))}.value{font-size:14px;color:rgb(var(--dyte-colors-text-700, 255 255 255 / 0.64));margin-top:4px}dyte-settings-video{display:flex;flex-direction:column}";
var DyteDebuggerVideoStyle0 = dyteDebuggerVideoCss;
...
var dyteDialogManagerCss = ":host{line-height:initial;font-family:var(--dyte-font-family, sans-serif);font-feature-settings:normal;font-variation-settings:normal}p{margin:var(--dyte-space-0, 0px);padding:var(--dyte-space-0, 0px)}:host{display:block}";
var DyteDialogManagerStyle0 = dyteDialogManagerCss;
var __decorate$z = function(decorators, target, key, desc) {
  var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
  if (typeof Reflect === "object" && typeof Reflect.decorate === "function")
    r = Reflect.decorate(decorators, target, key, desc);
...
var DyteDialogManager = class {
  ...
  render() {
    ...
    if ((states === null || states === void 0 ? void 0 : states.image) != null) {
      ...
      return h(Host, null, h("dyte-dialog", { open: true, onDyteDialogClose: onImageClose, hideCloseButton: true, iconPack: this.iconPack, t: this.t }, h("dyte-image-viewer", Object.assign({}, defaults, { image, onClose: onImageClose }))));
...
var DyteDraftAttachmentView = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
...
var EMOJI_ASSET_URL = "https://cdn.dyte.in/assets/emojis-data.json";
var cachedEmojis;
...
var DyteEmojiPicker = class {
  ...
  render() {
    return h(Host, { key: "332fe557a1fd6c4813a20dc67218583ed52f8080" }, h("div", { key: "c33a2a916b8bb69a6890de686837f26a2a52ca66", class: "close-parent" }, h("dyte-button", { key: "68957f915e70a98008dfe083e4cc1b5b01fcffb1", variant: "ghost", kind: "icon", class: "close", onClick: () => {
      var _a;
...
var dyteEmojiPickerButtonCss = ":host{}";
var DyteEmojiPickerButtonStyle0 = dyteEmojiPickerButtonCss;
var __decorate$w = function(decorators, target, key, desc) {
  var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
  if (typeof Reflect === "object" && typeof Reflect.decorate === "function")
    r = Reflect.decorate(decorators, target, key, desc);
...
var DyteEmojiPickerButton = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.iconPack = defaultIconPack;
    this.t = useLanguage();
...
var dyteFilePickerButtonCss = ":host{}";
var DyteFilePickerButtonStyle0 = dyteFilePickerButtonCss;
var __decorate$t = function(decorators, target, key, desc) {
  var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
  if (typeof Reflect === "object" && typeof Reflect.decorate === "function")
    r = Reflect.decorate(decorators, target, key, desc);
...
var DyteFilePickerButton = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
  ...
  render() {
    ...
    return h("dyte-tooltip", { key: "334c14777babe9de198038dd3b37926176d1a08a", label }, h("dyte-button", { key: "0bc4d797c62074d3ffe9531d04c9087b21b16e0f", variant: "ghost", kind: "icon", onClick: () => this.uploadFile(), title: label }, h("dyte-icon", { key: "99066f2ba8c7eb773b1e9d5d9895d340af1d11fe", icon })));
  }
};
__decorate$t([
  SyncWithStore()
...
var dyteImageMessageCss = ":host{line-height:initial;font-family:var(--dyte-font-family, sans-serif);font-feature-settings:normal;font-variation-settings:normal}p{margin:var(--dyte-space-0, 0px);padding:var(--dyte-space-0, 0px)}.image-spinner{cursor:wait}.image-errored{cursor:not-allowed}";
var DyteImageMessageStyle0 = dyteImageMessageCss;
var __decorate$r = function(decorators, target, key, desc) {
  var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
  if (typeof Reflect === "object" && typeof Reflect.decorate === "function")
    r = Reflect.decorate(decorators, target, key, desc);
...
var DyteImageMessageView = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
...
var DyteMeeting = class {
  ...
  connectedCallback() {
    ...
    this.resizeObserver.observe(this.host);
    if (this.applyDesignSystem && ((_a = this.config) === null || _a === void 0 ? void 0 : _a.designTokens) != null && typeof document !== "undefined") {
  ...
  meetingChanged(meeting) {
    ...
    if (this.loadConfigFromPreset && meeting.self.config != null) {
      ...
      if (meeting.connectedMeetings.supportsConnectedMeetings && ((_a = uiState.states.activeBreakoutRoomsManager) === null || _a === void 0 ? void 0 : _a.destinationMeetingId)) {
        this.showSetupScreen = false;
      }
    }
    if (this.applyDesignSystem && ((_b = this.config) === null || _b === void 0 ? void 0 : _b.designTokens) != null && typeof document !== "undefined") {
      provideDyteDesignSystem(document.documentElement, this.config.designTokens);
    }
  ...
  render() {
    ...
    return h(Render, { element: "dyte-meeting", defaults, asHost: true, elementProps });
  }
  get host() {
    return getElement(this);
  }
...
var dyteMicrophoneSelectorCss = `.dyte-select{--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-background-700, 44 44 44) / var(--tw-bg-opacity));color:rgb(var(--dyte-colors-text-1000, 255 255 255))}.dyte-select:focus{--tw-ring-offset-shadow:var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color);--tw-ring-shadow:var(--tw-ring-inset) 0 0 0 calc(0px + var(--tw-ring-offset-width)) var(--tw-ring-color);box-shadow:var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000)}.dyte-select{display:block;border-radius:var(--dyte-border-radius-sm, 4px);border-width:var(--dyte-border-width-none, 0);border-style:none;-webkit-appearance:none;-moz-appearance:none;appearance:none;padding:var(--dyte-space-3, 12px);font-size:16px;--icon-size:var(--dyte-select-chevron-size, var(--dyte-space-6, 24px));--icon-right-position:var(--dyte-select-chevron-right-position, var(--dyte-space-2, 8px));background-image:url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e");background-position:right var(--icon-right-position) center;background-repeat:no-repeat;background-size:var(--icon-size) var(--icon-size);padding-right:calc(var(--icon-right-position) * 5);width:100%;max-width:100%;text-overflow:ellipsis}.inline .dyte-select{margin-top:var(--dyte-space-1, 4px);width:100%;padding-top:var(--dyte-space-1, 4px);padding-bottom:var(--dyte-space-1, 4px);padding-left:var(--dyte-space-1\\.5, 6px);padding-right:var(--dyte-space-1\\.5, 6px);padding-right:var(--dyte-space-8, 32px);font-size:14px}.row{margin-bottom:var(--dyte-space-2, 8px);display:flex;width:100%;align-items:center;justify-content:space-between;gap:var(--dyte-space-3, 12px)}.group{margin-top:var(--dyte-space-2, 8px);margin-bottom:var(--dyte-space-2, 8px)}.group>*{margin-bottom:var(--dyte-space-2, 8px)}.group>*:last-child{margin-bottom:var(--dyte-space-0, 0px)}.group select{flex:1 1 0%}.inline.group{margin-top:var(--dyte-space-0, 0px);margin-bottom:var(--dyte-space-0\\.5, 2px)}.inline.group>*{margin-bottom:var(--dyte-space-0, 0px)}label{display:flex;-webkit-user-select:none;-moz-user-select:none;user-select:none;align-items:center;gap:var(--dyte-space-1, 4px);font-size:14px}.inline.container{display:flex;align-items:center;justify-content:flex-start;gap:var(--dyte-space-2, 8px);padding-left:var(--dyte-space-2, 8px);padding-right:var(--dyte-space-2, 8px)}`;
var DyteMicrophoneSelectorStyle0 = dyteMicrophoneSelectorCss;
var __decorate$g = function(decorators, target, key, desc) {
  var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
  if (typeof Reflect === "object" && typeof Reflect.decorate === "function")
    r = Reflect.decorate(decorators, target, key, desc);
...
var DyteMicrophoneSelector = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.iconPack = defaultIconPack;
    this.variant = "full";
...
var DyteMuteAllConfirmation = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.stateUpdate = createEvent(this, "dyteStateUpdate", 7);
    this.iconPack = defaultIconPack;
...
var DyteNotifications = class {
  ...
  render() {
    return h(Host, { key: "1e122b16897a90f7c7900057849e89d5152f477f" }, this.notifications.map((notification) => h("dyte-notification", { size: this.size, key: notification.id, "data-id": notification.id, notification, onDyteNotificationDismiss: (e) => this.handleDismiss(e), iconPack: this.iconPack, t: this.t })));
  }
  get host() {
    return getElement(this);
  }
...
var DytePermissionsMessage = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
    this.stateUpdate = createEvent(this, "dyteStateUpdate", 7);
    this.t = useLanguage();
...
var dyteSpeakerSelectorCss = `.dyte-select{--tw-bg-opacity:1;background-color:rgba(var(--dyte-colors-background-700, 44 44 44) / var(--tw-bg-opacity));color:rgb(var(--dyte-colors-text-1000, 255 255 255))}.dyte-select:focus{--tw-ring-offset-shadow:var(--tw-ring-inset) 0 0 0 var(--tw-ring-offset-width) var(--tw-ring-offset-color);--tw-ring-shadow:var(--tw-ring-inset) 0 0 0 calc(0px + var(--tw-ring-offset-width)) var(--tw-ring-color);box-shadow:var(--tw-ring-offset-shadow), var(--tw-ring-shadow), var(--tw-shadow, 0 0 #0000)}.dyte-select{display:block;border-radius:var(--dyte-border-radius-sm, 4px);border-width:var(--dyte-border-width-none, 0);border-style:none;-webkit-appearance:none;-moz-appearance:none;appearance:none;padding:var(--dyte-space-3, 12px);font-size:16px;--icon-size:var(--dyte-select-chevron-size, var(--dyte-space-6, 24px));--icon-right-position:var(--dyte-select-chevron-right-position, var(--dyte-space-2, 8px));background-image:url("data:image/svg+xml,%3csvg xmlns='http://www.w3.org/2000/svg' fill='none' viewBox='0 0 20 20'%3e%3cpath stroke='%236b7280' stroke-linecap='round' stroke-linejoin='round' stroke-width='1.5' d='M6 8l4 4 4-4'/%3e%3c/svg%3e");background-position:right var(--icon-right-position) center;background-repeat:no-repeat;background-size:var(--icon-size) var(--icon-size);padding-right:calc(var(--icon-right-position) * 5);width:100%;max-width:100%;text-overflow:ellipsis}.inline .dyte-select{margin-top:var(--dyte-space-1, 4px);width:100%;padding-top:var(--dyte-space-1, 4px);padding-bottom:var(--dyte-space-1, 4px);padding-left:var(--dyte-space-1\\.5, 6px);padding-right:var(--dyte-space-1\\.5, 6px);padding-right:var(--dyte-space-8, 32px);font-size:14px}.row{display:flex;width:100%;align-items:center;justify-content:space-between;gap:var(--dyte-space-3, 12px)}.group{margin-top:var(--dyte-space-2, 8px);margin-bottom:var(--dyte-space-2, 8px)}.group>*{margin-bottom:var(--dyte-space-2, 8px)}.group>*:last-child{margin-bottom:var(--dyte-space-0, 0px)}.group select{flex:1 1 0%}.inline.group{margin-top:var(--dyte-space-0, 0px);margin-bottom:var(--dyte-space-0\\.5, 2px)}.inline.group>*{margin-bottom:var(--dyte-space-0, 0px)}label{display:flex;-webkit-user-select:none;-moz-user-select:none;user-select:none;align-items:center;gap:var(--dyte-space-1, 4px);font-size:14px}.inline .container{display:flex;align-items:center;justify-content:flex-start;gap:var(--dyte-space-2, 8px);padding-left:var(--dyte-space-2, 8px);padding-right:var(--dyte-space-2, 8px)}`;
var DyteSpeakerSelectorStyle0 = dyteSpeakerSelectorCss;
...
var DyteSpinnerStyle0 = dyteSpinnerCss;
...
var DyteTextComposerView = class {
  constructor(hostRef) {
    registerInstance(this, hostRef);
...
export {
  DyteAudioVisualizer as dyte_audio_visualizer,
  DyteAvatar as dyte_avatar,
  DyteBreakoutRoomManager as dyte_breakout_room_manager,
  DyteBreakoutRoomParticipants as dyte_breakout_room_participants,
  DyteBreakoutRoomsManager as dyte_breakout_rooms_manager,
  DyteBroadcastMessageModal as dyte_broadcast_message_modal,
  DyteButton as dyte_button,
  DyteCameraSelector as dyte_camera_selector,
  DyteChannelCreator as dyte_channel_creator,
  DyteChannelDetails as dyte_channel_details,
  DyteChannelHeader as dyte_channel_header,
  DyteChannelSelectorView as dyte_channel_selector_view,
  DyteChat as dyte_chat,
  DyteChatComposerView as dyte_chat_composer_view,
  DyteChatMessage as dyte_chat_message,
  DyteChatMessagesUi as dyte_chat_messages_ui,
  DyteChatMessagesUiPaginated as dyte_chat_messages_ui_paginated,
  DyteChatSearchResults as dyte_chat_search_results,
  DyteConfirmationModal as dyte_confirmation_modal,
  DyteCounter as dyte_counter,
  DyteDebugger as dyte_debugger,
  DyteDebuggerAudio as dyte_debugger_audio,
  DyteDebuggerScreenShare as dyte_debugger_screenshare,
  DyteDebuggerSystem as dyte_debugger_system,
  DyteDebuggerVideo as dyte_debugger_video,
  DyteDialog as dyte_dialog,
  DyteDialogManager as dyte_dialog_manager,
  DyteDraftAttachmentView as dyte_draft_attachment_view,
  DyteEmojiPicker as dyte_emoji_picker,
  DyteEmojiPickerButton as dyte_emoji_picker_button,
  DyteFileMessage as dyte_file_message,
  DyteFileMessageView as dyte_file_message_view,
  DyteFilePickerButton as dyte_file_picker_button,
  DyteIcon as dyte_icon,
  DyteImageMessage as dyte_image_message,
  DyteImageMessageView as dyte_image_message_view,
  DyteImageViewer as dyte_image_viewer,
  DyteJoinStage as dyte_join_stage,
  DyteLeaveMeeting as dyte_leave_meeting,
  DyteLogo as dyte_logo,
  DyteMarkdownView as dyte_markdown_view,
  DyteMeeting as dyte_meeting,
  DyteMenu$1 as dyte_menu,
  DyteMenuItem as dyte_menu_item,
  DyteMenuList as dyte_menu_list,
  DyteMessageView as dyte_message_view,
  DyteMicrophoneSelector as dyte_microphone_selector,
  DyteMuteAllConfirmation as dyte_mute_all_confirmation,
  DyteNotification as dyte_notification,
  DyteNotifications as dyte_notifications,
  DyteOverlayModal as dyte_overlay_modal,
  DytePaginatedList as dyte_paginated_list,
  DyteParticipantTile as dyte_participant_tile,
  DytePermissionsMessage as dyte_permissions_message,
  DyteSettings as dyte_settings,
  DyteSettingsAudio as dyte_settings_audio,
  DyteSettingsVideo as dyte_settings_video,
  DyteSpeakerSelector as dyte_speaker_selector,
  DyteSpinner as dyte_spinner,
  DyteSwitch as dyte_switch,
  DyteTextComposerView as dyte_text_composer_view,
  DyteTextMessage as dyte_text_message,
  DyteTextMessageView as dyte_text_message_view,
  DyteMenu as dyte_tooltip
};
//# sourceMappingURL=dyte-audio-visualizer_64.entry-MRWA7CHC.js.map
