...
const SignupPanel: React.FC<SignupPanelProps> = ({
  ...
  const [role, setRole] = useState<"student" | "teacher">("student"); // 👈 role state
  const [firstName, setFirstName] = useState("");
  const [lastName, setLastName] = useState("");
  const [email, setEmail] = useState("");
  const [mobile, setMobile] = useState("");
  ...
  const validateForm = () => {
    if (!firstName || !lastName || !email || !mobile || !password || !passwordConfirmation || !selectedBoard) {
      setError("All fields are required.");
      return false;
    }
  ...
  const handleSignup = async () => {
    ...
    try {
      ...
      if (role === "student") {
        url = "http://lms.hawc.in/api/register/student";
        requestData.class_id = selectedClass; // students need class_id
      } else {
        url = "http://lms.hawc.in/api/register/lecturer";
      }
    ...
    <>
      ...
      >
        <div className="flex items-center p-4 mt-9">
          ...
          <h2 className="text-2xl font-semibold text-gray-900 dark:text-white text-left w-full mt-10">
            Sign Up
          </h2>
        </div>
        ...
        <div className="flex flex-col items-center justify-start flex-1 p-4 gap-3 -mt-4">
          {error && (
            <p className="text-red-500 text-xs p-2 bg-red-50 rounded-md w-64 sm:w-72 md:w-80 whitespace-pre-line">
              {error}
            </p>
          )}
          {message && (
            <p className="text-green-600 text-xs p-2 bg-green-50 rounded-md w-64 sm:w-72 md:w-80">
              {message}
            </p>
          )}

          {/* ✅ Role Selection */}
          <div className="flex gap-4 w-64 sm:w-72 md:w-80 mb-2">
            <div
              onClick={() => setRole("student")}
              className={`flex-1 p-3 cursor-pointer rounded-lg border text-center ${
                role === "student"
                  ? "bg-gray-800 text-white border-gray-800"
                  : "bg-gray-100 dark:bg-gray-800 border-gray-300 dark:border-gray-700 text-gray-700 dark:text-gray-300"
              }`}
            >
              <FaUserGraduate className="mx-auto mb-1" />
              Student
            </div>
            <div
            ...
            >
              <FaChalkboardTeacher className="mx-auto mb-1" />
              Teacher
            </div>
          </div>
          ...
          <div className="flex gap-4 w-64 sm:w-72 md:w-80">
            <div className="flex-1">
              ...
              <select
                value={selectedBoard}
                onChange={(e) =>
                  setSelectedBoard(e.target.value ? Number(e.target.value) : "")
                }
                className="w-full p-2 text-sm rounded border border-gray-300 dark:border-gray-700 bg-gray-100 dark:bg-gray-800 text-gray-900 dark:text-white"
              >
                <option value="">Select Board</option>
            ...
            {role === "student" && ( // 👈 class only for students
              <div className="flex-1">
                <label className="text-gray-700 dark:text-gray-300 text-xs self-start">
                  Select Class
                </label>
