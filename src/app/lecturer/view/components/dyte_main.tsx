...
const DyteMeetingPage: React.FC = () => {
  ...
  const SERVER_URL = "https://livesocket.hawc.in";
  ...
  useEffect(() => {
    const fetchServerConfig = async () => {
      try {
        ...
        setServerConfig({ websocket: "wss://livesocket.hawc.in" });
      }
    };
  ...
  useEffect(() => {
    if (!meetingId || !serverConfig) return;
    const socket = new WebSocket(serverConfig.websocket);
    setWs(socket);

    socket.onopen = () => {
      console.log("✅ WebSocket connected to:", serverConfig.websocket);
      socket.send(JSON.stringify({ 
        type: "join_notification", 
        meetingId, 
        userRole: "teacher"
    ...
    socket.onmessage = (event) => {
      try {
        ...
        if ((data.type === "PAUSE_TOGGLE" || data.type === "PAUSE_STATE") && data.meetingId === meetingId) {
          setIsPaused(prev => {
            console.log(`🔄 Pause state updated from WebSocket → ${data.isPaused ? "⏸️ Paused" : "▶️ Resumed"}`);
            console.log(prev);
  ...
  const togglePause = async () => {
    try {
      ...
      if (!isPaused) {
        console.log("⏸️ Sending pause request...");
        const response = await axios.post(`${SERVER_URL}/api/pause`, { 
          meetingId, 
          name: "teacher" 
        });
        ...
        console.log("▶️ Sending resume request...");
        const response = await axios.post(`${SERVER_URL}/api/resume`, { 
          meetingId, 
          name: "teacher" 
        });
  ...
  if (!meeting || !classInfo) {
    return (
      <div className="flex items-center justify-center min-h-screen bg-[#141414]">
        <div className="text-center p-8 max-w-[90%] w-auto">
          <div className="relative mx-auto mb-8 w-[150px] h-[150px]">
            <div className="absolute top-0 left-0 w-full h-full rounded-full border-10 border-t-10 border-blue-400 border-opacity-20 animate-spinLoader"></div>
            <div className="absolute top-1/2 left-1/2 w-[70px] h-[70px] rounded-full bg-white flex items-center justify-center -translate-x-1/2 -translate-y-1/2 overflow-hidden">
              <img src={logo} alt="Loader Logo" className="w-[80%] h-[80%] object-contain" />
  ...
  return (
    <DyteProvider value={meeting}>
      <DyteUiProvider>
        <div className="flex flex-col bg-black min-h-screen mb-20 md:mb-0">
