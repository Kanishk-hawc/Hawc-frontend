...
const Header: React.FC<HeaderProps> = ({ toggleTheme, isDarkMode }) => {
  return (
    <header className={`${isDarkMode ? 'bg-black' : 'bg-white'} text-${isDarkMode ? 'white' : 'gray-900'} transition-colors duration-300 p-4 flex justify-between items-center`}>
      ...
      >
        <div className="absolute inset-0 rounded-full overflow-hidden">
          <div className={`absolute inset-0 bg-gradient-to-r from-yellow-200 to-blue-500 opacity-20 dark:opacity-30 transition-opacity duration-300`} />
        </div>
        <div
